      ...
      {fields.map((field, index) => (
        <Paper key={field.id} sx={{ p: 3, borderRadius: 4, border: "1px solid #e5e7eb" }}>
          <Stack direction="row" sx={{ justifyContent: "space-between", mb: 2 }}>
            <Typography sx={{ fontWeight: 900 }}>Language #{index + 1}</Typography>
            {fields.length > 1 && <IconButton color="error" onClick={() => remove(index)}><Delete /></IconButton>}
          </Stack>

          <Grid container spacing={2}>
            <Grid size={{ xs: 12, md: 6 }}>
              <AppTextField name={`languages.${index}.name`} control={control} label="Language" />
            </Grid>
            <Grid size={{ xs: 12, md: 6 }}>
              <AppTextField name={`languages.${index}.proficiency`} control={control} label="Proficiency" placeholder="Fluent / Native" />
            </Grid>
          </Grid>
        </Paper>
      ))}

      <Button variant="outlined" startIcon={<Add />} onClick={() => append({ name: "", proficiency: "" })}>
        Add Language
      </Button>
    </Stack>
  );
